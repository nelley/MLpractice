...
#設定去除掉的詞的規則:min_d詞出現低於10份文件的去掉 max_df詞頻大於0.5去掉
#float means frequency of the word
#int means 最少要出現在幾份文件裡
vectorizer = StemmedTfidfVectorizer(min_df=10, max_df=0.5,
                                    stop_words='english', decode_error='ignore'
                                    )
# learn the vocabulary dictionary and return the term-document matrix
vectorized = vectorizer.fit_transform(train_data.data)
print vectorized.getrow(4)    #print corresponding non-zero feature vector, train_data裡有幾份文件就有多少row
